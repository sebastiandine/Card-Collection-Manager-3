...
2. **Auto-detect print (`detectFirstPrint`, edit dialog).** Uses `fname=` plus **`cardset=`** set to the **display set name** from the picker (must match `card_sets[].set_name` in the payload). If that request fails (for example unknown set label), it retries with **`fname=` only** and still filters prints in `parseFirstPrint` by preferred `set_name`. No image data is needed for this path, so Yugipedia is not consulted.
...
- Yu-Gi-Oh!: none configured (`fallbackImageUrlForGame` returns empty), so a failed preview stays on the empty placeholder unless that changes.
