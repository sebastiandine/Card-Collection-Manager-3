...
JapanesePokemonCardEditDialog::JapanesePokemonCardEditDialog(wxWindow* parent,
    ...
    : BaseCardEditDialog<JapanesePokemonCard>(
          parent,
          mode == EditMode::Create ? "Add Pokemon (Japan) Card" : "Edit Pokemon (Japan) Card",
          imageService, setService, mode, std::move(initial), Game::JapanesePokemon, preloadedSets),
      dialogMode_(mode),
...
JapanesePokemonCardEditDialog::~JapanesePokemonCardEditDialog() {
    if (variantFetchState_) {
        variantFetchState_->alive.store(false);
    }
}

void JapanesePokemonCardEditDialog::onCardLookupContextChanged() {
    clearCachedPrintVariants();
}

void JapanesePokemonCardEditDialog::buildFlagsRow(wxBoxSizer* flagsBox) {
...
void JapanesePokemonCardEditDialog::appendExtraRows(wxFlexGridSizer* grid) {
    ...
    autoSetNoBtn_->Bind(wxEVT_BUTTON, &JapanesePokemonCardEditDialog::onAutoDetectSetNo, this);
    nextSetNoBtn_ = new wxButton(setNoPanel, wxID_ANY, "Next");
    nextSetNoBtn_->Bind(wxEVT_BUTTON, &JapanesePokemonCardEditDialog::onNextSetNo, this);
    ...
    if (auto* setCombo = setComboControl()) {
        setCombo->Bind(wxEVT_COMBOBOX, &JapanesePokemonCardEditDialog::onSetSelectionChanged, this);
    }
}

std::string JapanesePokemonCardEditDialog::normalizedStoredSetNo(std::string_view setNo) {
    return JapanesePokemonCardPreviewSource::normalizeLocalId(setNo);
}

std::string JapanesePokemonCardEditDialog::storedSetNoFromControls(const wxTextCtrl* ctrl) {
    if (ctrl == nullptr) return {};
    return normalizedStoredSetNo(ctrl->GetValue().ToStdString(wxConvUTF8));
}

void JapanesePokemonCardEditDialog::readExtraFromCard() {
    clearCachedPrintVariants();
    if (setNoCtrl_) {
...
void JapanesePokemonCardEditDialog::writeExtraToCard() {
...
void JapanesePokemonCardEditDialog::clearCachedPrintVariants() {
    ++variantFetchEpoch_;
...
void JapanesePokemonCardEditDialog::scheduleDeferredVariantPrefetch() {
...
void JapanesePokemonCardEditDialog::prefetchVariantsForCurrentCardSilent(unsigned capturedEpoch) {
...
void JapanesePokemonCardEditDialog::requestVariantsAsync(unsigned capturedEpoch,
                                                 std::string name,
                                                 std::string setId,
...
void JapanesePokemonCardEditDialog::applyDetectedVariants(unsigned capturedEpoch,
...
void JapanesePokemonCardEditDialog::rebuildVariantRingFromCache() {
    uniqueSetNos_.clear();
    if (cachedVariants_.empty()) return;
...
void JapanesePokemonCardEditDialog::syncRingPositionToControls() {
...
void JapanesePokemonCardEditDialog::refreshVariantNextControls() {
...
void JapanesePokemonCardEditDialog::onAutoDetectSetNo(wxCommandEvent&) {
    autoDetectFromApi();
}

void JapanesePokemonCardEditDialog::onNextSetNo(wxCommandEvent&) {
...
void JapanesePokemonCardEditDialog::autoDetectFromApi() {
...
void JapanesePokemonCardEditDialog::onSetSelectionChanged(wxCommandEvent& ev) {
    clearCachedPrintVariants();
    scheduleDeferredVariantPrefetch();
    ev.Skip();
}
